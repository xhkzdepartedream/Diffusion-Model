...
class sampler:
    def __init__(self, model, device, use_cos: bool, prediction_type: str = 'epsilon'):
        self.n_steps = 1000
        self.shape = (1, 3, 64, 64)
        self.device = device
        self.model = model
        if hasattr(model, 'learn_sigma'):
            self.learn_sigma = model.learn_sigma
        else:
            self.learn_sigma = False
        if use_cos:
            self.diffusion = CosineDenoiseDiffusion(self.model, self.n_steps, device = device)
        else:
            self.diffusion = DenoiseDiffusion(self.model, self.n_steps, device = device, prediction_type = prediction_type)
    ...
    def visualize_denoising_process(self, steps: int, shape: Tuple[int, int, int, int],
        ...
        with torch.no_grad():
            ...
            images = [xt.clone()]
            # assert xt.device == self.model.device
