...
from diffusion_and_unet.DenoiseDiffusion import DenoiseDiffusion, CosineDenoiseDiffusion
from utils import *
from .DiT import DiT
...
class DiT_trainer:
    def __init__(self, dataset: Dataset, n_steps: int, batch_size: int, lr: float, n_epoch: int, use_cos: bool,
                 ...
                 num_heads: int = 16, mlp_ratio: int = 4, learn_sigma: bool = True, attn_dropout = 0.0,
                 mlp_dropout = 0.0):
        ...
        self.eps_model = DiT(input_size, patch_size, input_ch, n_ch, n_blocks, num_heads, mlp_ratio,
                             learn_sigma = learn_sigma, attn_dropout = attn_dropout, mlp_dropout = mlp_dropout).to(
            device)
